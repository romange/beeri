...
// Author: romange@google.com (Roman Gershman)
...
static bool IsPrime(uint32 val) {
...
uint32 NextHigherPrime(uint32 value) {
  if (value % 2 == 0) ++value;
  for (; value < kuint32max ; value += 2) {
    if (IsPrime(value)) {
      return value;
    }
  }
...
inline bool is_power_2(uint32 v) {
  return ((v - 1) & v) == 0;
}
...
public:
  ...
  uint8 num_children() const { return Bits::CountOnesInByte(children_mask()); }

  // returns first unexplored child.
  uint8 first_child() const {
    DCHECK_GT(num_children(), 0);
    return Bits::FindLSBSetNonZero(children_mask());
  }
...
inline CuckooMapTable::BucketId CuckooMapTable::NextBucketId(
  ...
  DCHECK(id_pair.first == current || id_pair.second == current);
  DCHECK_NE(id_pair.first, id_pair.second);
  return (current == id_pair.first) ? id_pair.second : id_pair.first;
}

CuckooMapTable::BucketIdPair CuckooMapTable::HashToIdPair(const key_type v) const {
  uint32 a = hash1(v);
  __builtin_prefetch(buf_.get() + a * bucket_size_, 0, 1);
...
inline CuckooMapTable::dense_id CuckooMapTable::FindInBucket(
  ...
  const key_type* a = GetBucketById(id_pair.first)->key;
  for (uint8 i = 0; i < kBucketLength; ++i) {
    if (a[i] == k) return ToDenseId(id_pair.first, i);
  }
  a = GetBucketById(id_pair.second)->key;
  for (uint8 i = 0; i < kBucketLength; ++i) {
    if (a[i] == k) return ToDenseId(id_pair.second, i);
  }
  return npos;
}
...
std::pair<CuckooMapTable::dense_id, bool> CuckooMapTable::Insert(key_type k, const uint8* data) {
  ...
  uint32 iteration = 0;
  while (true) {
    VLOG(3) << "insert iteration: " << iteration << ", pending = " << pending_key_;
...
inline int CuckooMapTable::InsertIntoBucket(const uint32 empty_mask, Bucket* bucket) {
  int i = Bits::FindLSBSetNonZero(empty_mask);
...
void CuckooMapTable::SetEmptyValues() {
  uint8* ptr = buf_.get();
  for (uint32 i = 0; i < bucket_count_; ++i, ptr += bucket_size_) {
    Bucket* bucket = reinterpret_cast<Bucket*>(ptr);
    std::fill(bucket->key, bucket->key + kBucketLength, empty_value_);
  }
}

void CuckooMapTable::Init(uint32 bucket_capacity) {
  static_assert(((kBucketLength - 1) & kBucketLength) == 0, "BucketSizeMustBePowerOf2");
  ...
  bucket_count_ = NextHigherPrime(bucket_capacity);
  DoAllocate();
}
...
CuckooMapTable::dense_id CuckooMapTable::RollPending(uint32 shifts_limit,
  ...
  Bucket* bucket1 = GetBucketById(bucket_pair.first);
  Bucket* bucket2 = GetBucketById(bucket_pair.second);

  uint32 empty[2] = { CheckEmpty(*bucket1), CheckEmpty(*bucket2) };
  if (empty[0] + empty[1] == 0) {
    BucketId bid = bucket_pair.first;
    if (random_bit(random_bit_indx_)) {
      bid = bucket_pair.second;
      bucket1 = bucket2;
    }
    ...
    VLOG(2) << "Inserting " << pending_key_ << " into bucket1 with mask " << empty[0];
    // There is an empty slot in the first bucket.
    return ToDenseId(bucket_pair.first, InsertIntoBucket(empty[0], bucket1));
  }
  int index = InsertIntoBucket(empty[1], bucket2);
  VLOG(2) << "Inserting " << pending_key_ << " into bucket2 " << bucket_pair.first
          << " index= " << index << " with mask " << empty[1];

  // There is an empty slot in the second bucket.
  return ToDenseId(bucket_pair.second, index);
}

void CuckooMapTable::Grow() {
  ...
  VLOG(1) << "PreGrow: " << "size_ = " << size_ << ", capacity= "
  ...
  while (true) {
    uint32 new_bucket_count = static_cast<uint32>(bucket_count_ * growth_ + 1.0f);
    new_bucket_count = NextHigherPrime(new_bucket_count);
    CHECK_GT(new_bucket_count, bucket_count_);
    bucket_count_ = new_bucket_count;
    DoAllocate();

    VLOG(1) << "Growing iteration: " << iteration << ", new_capacity "
            << Capacity() << ", new utilization " << Utilization();
    // Copy old the existing data to the new storage.
    bool success = CopyBuffer(old_buf.get(), old_bucket_count,
        [this](const BucketIdPair& pair) {
...
void CuckooMapTable::DoAllocate() {
  buf_.reset(new uint8[bucket_count_* bucket_size_]);
  SetEmptyValues();
  shifts_limit_ = Bits::Log2FloorNonZero64(bucket_count_) * 2;
  divide_s_ = libdivide::libdivide_u64_gen(bucket_count_);
  divide_s_alg_ = libdivide::libdivide_u64_get_algorithm(&divide_s_);
  // CHECK_EQ(1, ) << bucket_count_;
}
...
bool CuckooMapTable::Compact(double ratio) {
  ...
  for (int i = 0; i < kMaxCompactTries; ++i) {
    ...
    desired_bucket_size = NextHigherPrime(desired_bucket_size + 2);
  }
  return false;
}
...
bool CuckooMapTable::InsertProblematicKeys(const std::vector<key_type>& keys, const uint8* values) {
  ...
  for (key_type k : keys) {
    ...
    BucketIdPair id_pair = HashToIdPair(pending_key_);
    if (!ShiftExhaustive(id_pair.first)) {
      return false;
    }
    values += value_size_;
  }
...
bool CuckooMapTable::ShiftExhaustive(BucketId bid) {
  ...
  while (!compaction_stack_.empty()) {
    ...
    #if ADDITIONAL_CHECKS || !defined NDEBUG
    {
     ...
     CHECK(key.first == parent || key.second == parent)
            << parent << ", " << pending_key_;
    }
    #endif
...
#if 0
void CuckooMapTable::CopyFrom(const CuckooMapTable& other) {
    if (this == &other) return;
    size_ = other.size_;
    empty_value_ = other.empty_value_;
    empty_value_set_ = other.empty_value_set_;
    bucket_count_ = other.bucket_count_;
    growth_ = other.growth_;
    max_evictions_ = other.max_evictions_;
    memory_.reset(new Bucket[bucket_count_]);
    std::copy(other.memory_.get(), other.memory_.get() + bucket_count_,
              memory_.get());
  }
#endif
}  // namespace base
