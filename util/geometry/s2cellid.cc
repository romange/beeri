...
string S2CellId::ToString() const {
  if (!is_valid()) {
    return StringPrintf("Invalid: %016" PRIu64 "x", id());
  }
...
S2CellId S2CellId::ParseFromString(const std::string& str) {
  if (str.size() < 2 || str.size() > S2CellId::kMaxLevel + 2 ||
      !ascii_isdigit(str[0]) || str[1] != '/')
    return None();
  int level = 0;
  int face = str[0] - '0';
  uint64 pos = 0;
  ...
  pos <<= (2*(S2CellId::kMaxLevel - level) + 1);
