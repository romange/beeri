...
bool S2Loop::IsValid() const {
  ...
  std::unordered_map<S2Point, int> vmap;
  for (int i = 0; i < num_vertices(); ++i) {
    if (!vmap.insert(make_pair(vertex(i), i)).second) {
      VLOG(2) << "Duplicate vertices: " << vmap[vertex(i)] << " and " << i << " out of "
              << num_vertices();
      VLOG(2) << "Vertex value: " << vertex(i) << " or " << S2LatLng(vertex(i));
...
bool S2Loop::IsValid(vector<S2Point> const& vertices, int ) {
