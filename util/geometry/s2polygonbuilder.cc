...
bool S2PolygonBuilder::AddEdge(S2Point const& v0, S2Point const& v1) {
  ...
  VLOG(2) << "AddEdge: " << S2LatLng(v0) << "->" << S2LatLng(v1);
  ...
  VLOG(2) << "AddEdge: Added " << S2LatLng(v0) << "->" << S2LatLng(v1) << ", edges_size: "
          << edges_->size();
  return true;
}
...
void S2PolygonBuilder::EraseEdge(S2Point const& v0, S2Point const& v1) {
  ...
  VLOG(2) << "EraseEdge: " <<  S2LatLng(v0) << "->" << S2LatLng(v1) << ", undirected: "
...
S2Loop* S2PolygonBuilder::AssembleLoop(S2Point const& v0, S2Point const& v1,
  ...
  std::unordered_map<S2Point, int> index;  // Maps a vertex to its index in "path".
  path.push_back(v0);
  path.push_back(v1);
  index[v1] = 1;
  while (path.size() >= 2) {
...
void S2PolygonBuilder::BuildMergeMap(PointIndex* index, MergeMap* merge_map) {
  ...
  std::unordered_set<S2Point> vertices;
  VLOG(1) << "BuildMergeMap: edges_size " << edges_->size();
  for (EdgeSet::const_iterator i = edges_->begin(); i != edges_->end(); ++i) {
  ...
  for (auto vstart = vertices.begin(); vstart != vertices.end(); ++vstart) {
...
void S2PolygonBuilder::MoveVertices(MergeMap const& merge_map) {
  ...
  VLOG(1) << "MoveVertices: total " << edges_->size() << ", moving " << edges.size();
...
bool S2PolygonBuilder::AssembleLoops(vector<S2Loop*>* loops,
  ...
  unused_edges->clear();
  VLOG(1) << "Stating vertices " << starting_vertices_.size() << ", edges: " << edges_->size();
...
bool S2PolygonBuilder::AssemblePolygon(S2Polygon* polygon,
  ...
  VLOG(1) << "Assembled " << loops.size() << " loops, success: " << success;
