...
void GetVertices(char const* str, Matrix3x3_d const& m,
  ...
  std::unique_ptr<S2Polyline> line(S2Testing::MakePolyline(str));
...
void DumpUnusedEdges(vector<pair<S2Point, S2Point> > const& unused_edges,
  ...
  fprintf(stderr,
          "Wrong number of unused edges (%d expected, %lu actual):\n",
...
void PrintLoop(std::ostream& os, const S2Loop& loop) {
  for (int i = 0; i < loop.num_vertices(); ++i) {
    const S2Point& p = loop.vertex(i);
    S2LatLng latlng(p);
    os << latlng.lat().degrees() << ", " << latlng.lng().degrees();
    if (i + 1 < loop.num_vertices())
      os << ", ";
  }
  os << endl;
}
...
TEST(S2PolygonBuilder, Basic) {
  std::vector<uint64> cell_ids({1152921504606846976ULL, 3458764513820540928ULL,
                                5764607523034234880ULL, 8070450532247928832ULL,
                                10376293541461622784ULL, 12682136550675316736ULL});
  S2PolygonBuilder builder(S2PolygonBuilderOptions::UNDIRECTED_UNION());
  for (uint64 val : cell_ids) {
    S2Cell cell{S2CellId(val)};
    CHECK(cell.id().is_valid());
    LOG(INFO) << S2CellId(val);
    for (int i = 0; i < 4; ++i) {
      LOG(INFO) << "v: " << S2LatLng(cell.GetVertex(i));
    }
    S2Loop cell_loop{cell};

    PrintLoop(COMPACT_GOOGLE_LOG_INFO.stream(), cell_loop);
    ASSERT_EQ(1, cell_loop.sign());
    builder.AddLoop(&cell_loop);
  }
  S2Polygon polygon;
  builder.AssemblePolygon(&polygon, NULL);
  EXPECT_GT(polygon.num_loops(), 0);
  LOG(INFO) << "OutLoops:";
  // for (int i = 0; i < polygon.num_loops(); ++i)
  //  PrintLoop(COMPACT_GOOGLE_LOG_INFO.stream(), *polygon.loop(i));
}
